    ...
    <div class="dashboard-grid">
      <div class="main-content">
        ...
        <div class="tabs">
          ...
          <button class="tab-btn" data-tab="cuenta">Estado de Cuenta</button>
        </div>
...
<script src="panel-jefe.js"></script>
  ...
  <!-- NUEVAS FUNCIONALIDADES AGREGADAS PARA JEFE DE VENTAS -->
  ...
  <section class="container">
    ...
    <form id="formProducto">
      ...
      Precio Unitario: <input type="number" id="precio"><br>
    ...
    <button onclick="verEstadoPorFecha()" class="btn btn-info">Por Fecha</button>
    <button onclick="verEstadoPorCliente()" class="btn btn-info">Por Cliente</button>
    <div id="estadoCuenta"></div>

    <h2>Anular Pedido</h2>
    <input type="text" id="idAnularPedido" placeholder="ID del pedido a anular">
    <button onclick="anularPedido()" class="btn btn-danger">Anular</button>
  </section>
  ...
  <script>
    ...
    document.getElementById("formProducto").addEventListener("submit", function(e) {
      ...
      }).then(() => alert("Producto cargado correctamente."));
    });
    ...
    db.collection("productos").onSnapshot(snapshot => {
      ...
      snapshot.forEach(doc => {
        let data = doc.data();
        lista.innerHTML += `<li>${data.codigo} - ${data.descripcion} - $${data.precio}</li>`;
      });
    });
    ...
    db.collection("pedidos").where("estado", "==", "pendiente").onSnapshot(snapshot => {
      ...
      snapshot.forEach(doc => {
        let data = doc.data();
        lista.innerHTML += `<li>${doc.id} - Cliente: ${data.cliente} - Total: $${data.total}
        <button onclick="entregarPedido('${doc.id}')">Entregar</button></li>`;
      });
    });
    ...
    function entregarPedido(id) {
      db.collection("pedidos").doc(id).get().then(doc => {
        const data = doc.data();
        db.collection("pedidos_entregados").doc(id).set(data);
        db.collection("pedidos").doc(id).delete();
        alert("Pedido entregado");
      });
    }

    function verEstadoPorFecha() {
      db.collection("facturas").orderBy("fecha").get().then(snapshot => {
        mostrarEstado(snapshot);
      });
    }

    function verEstadoPorCliente() {
      db.collection("facturas").orderBy("cliente").get().then(snapshot => {
        mostrarEstado(snapshot);
      });
    }

    function mostrarEstado(snapshot) {
      let html = "";
      snapshot.forEach(doc => {
        const d = doc.data();
        html += `<p>Cliente: ${d.cliente} | Fecha: ${d.fecha} | Total: $${d.total}</p>`;
      });
      document.getElementById("estadoCuenta").innerHTML = html;
    }

    function anularPedido() {
      const id = document.getElementById("idAnularPedido").value;
      db.collection("pedidos").doc(id).delete().then(() => {
        alert("Pedido anulado");
      });
    }

    // Validación y registro simulado
    function validarCliente(clienteId) {
      return db.collection("clientes").doc(clienteId).get().then(doc => doc.exists);
    }

    function registrarVenta(pedidoId, cliente, total, articulos) {
